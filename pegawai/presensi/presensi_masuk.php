...
/* --- 2. CEK BIOMETRIK (ALERT DIPERCANTIK) --- */
...
// Jika wajah belum terdaftar, tampilkan SweetAlert cantik
if (empty($descriptor_db)) {
    ...
    <head>
        ...
        <title>Biometrik Dibutuhkan</title>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    html: 'Anda harus melakukan <b>perekaman wajah</b> terlebih dahulu sebelum bisa melakukan presensi.',
                    confirmButtonText: 'Daftar Sekarang',
                    confirmButtonColor: '#206bc4', // Warna Biru (Primary)
                    allowOutsideClick: false,
                    allowEscapeKey: false
                }).then((result) => {
                    if (result.isConfirmed) {
                        window.location.href = '../face-recg/registrasi_wajah.php';
                    }
                });
            });
        </script>
...
/* --- 3. TANGKAP KOORDINAT & VALIDASI RADIUS (SERVER-SIDE) --- */
// Pastikan akses dari form Home (bukan ketik URL)
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: ../home/home.php"); exit;
}

// =========================================================================
// --- SATPAM WAKTU: BLOKIR JIKA MEMBOBOL TOMBOL SEBELUM WAKTUNYA ---
// =========================================================================
$lokasi_presensi = $_SESSION['user']['lokasi_presensi'] ?? '';
$query_lokasi = mysqli_query($conn, "SELECT jam_masuk, zona_waktu FROM lokasi_presensi WHERE nama_lokasi = '$lokasi_presensi'");
...
if ($data_lokasi) {
    ...
    // Cek Jam Server vs Jam Masuk Kantor
    $waktu_sekarang = time();
    $jam_masuk_master = $data_lokasi['jam_masuk'];
    $jamMasukToday = strtotime(date('Y-m-d') . ' ' . $jam_masuk_master);
    $waktu_buka_absen = $jamMasukToday - (30 * 60); // Buka 30 menit sebelum
    ...
    if ($waktu_sekarang < $waktu_buka_absen) {
        ...
        <body><script>
            Swal.fire({
                title: 'Akses Ilegal!',
                html: 'Belum waktunya presensi masuk.',
                icon: 'error',
                allowOutsideClick: false
...
// --- TAMBAHAN PENTING: TANGKAP AKURASI ---
$accuracy     = (float)($_POST['accuracy'] ?? 0);
// Cek Anti-Fake GPS (Jika akurasi terlalu sempurna/kecil, tolak)
if ($accuracy <= 1.0 && $accuracy > 0) {
    ...
    <body><script>
        Swal.fire({
            ...
            text: 'Akurasi GPS Anda terlalu sempurna (<?= $accuracy ?>m). Mohon matikan Fake GPS.',
            icon: 'error',
...
// Proteksi Radius: Jika tembus JS, blokir di PHP
...
<script>
 ...
 <style>
    ...
    .card-bio { border-radius: 20px; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.05); }
   ...
   /* 1. Pembungkus Kamera */
    #cam-box { 
        ...
        background: #000; 
        aspect-ratio: 3/4; 
        border: 5px solid #fff; /* Kembalikan border putih */
        box-shadow: 0 8px 24px rgba(0,0,0,0.15); /* Tambahan bayangan elegan */
        width: 100%;
        max-width: 400px; /* CEGAH KAMERA RAKSASA DI LAPTOP */
    ...
    /* Pembungkus utama agar oval berada di tengah (OPSIONAL) */
    .face-overlay {
        position: absolute;
        inset: 0; /* Memenuhi cam-box */
        display: flex;
        align-items: center;
        justify-content: center;
        z-index: 10;
        pointer-events: none; /* Agar tidak menghalangi klik jika ada */
    }

    /* Lubang Oval (Face Guide) (OPSIONAL)*/
    .face-cutout {
        width: 60%;
        height: 55%;
        border-radius: 50%; /* Bentuk Oval */
        /* Rahasia layar gelap bolong di tengah: */
    ...
    /* MAPS LETLEAF */
    #map-box { height: 320px; border-radius: 15px; border: 1px solid #e6e8eb; }
</style>
            ...
            <div class="col-lg-5">
                <div class="card card-bio">
                    <div class="card-header">
                        <h3 class="card-title text-primary fw-bold"><i class="ti ti-login me-2"></i> Konfirmasi Masuk</h3>
                    </div>
                    <div class="card-body text-center">
                        ...
                        <!--  -->
                        <button id="btnSubmit" class="btn btn-primary btn-lg w-100 mt-4 shadow-sm" disabled>
                            Ambil Foto & Masuk
                        </button>
                    </div>
                </div>
                        ...
                        <div class="row g-2 mb-3 text-center">
                            <div class="col-6">
                                <div class="p-2 border rounded bg-light">
                                    ...
                                    <div class="h3 mb-0 text-primary" id="liveClockMasuk"><?= $jamMasuk ?></div>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="p-2 border rounded bg-light">
...
<form id="formAction" method="post" action="presensi_masuk_aksi.php">
    <input type="hidden" name="foto_base64" id="foto_base64">
    <input type="hidden" name="latitude_masuk" value="<?= $latPegawai ?>">
    <input type="hidden" name="longitude_masuk" value="<?= $lngPegawai ?>">
    <input type="hidden" name="accuracy" value="<?= $accuracy ?>"> <!-- OPSIONAL -->
    <!-- jam masuk di hilangkan -->
</form>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const serverTimeMsMasuk = <?= time() * 1000 ?>;
    const startPerfMasuk = performance.now();
    
    // Ambil zona waktu dari PHP (default Asia/Jakarta)
    const tzString = '<?= ($zona === "WITA") ? "Asia/Makassar" : (($zona === "WIT") ? "Asia/Jayapura" : "Asia/Jakarta") ?>';

    setInterval(() => {
        const now = new Date(serverTimeMsMasuk + (performance.now() - startPerfMasuk));
        ...
        const clockUI = document.getElementById('liveClockMasuk');
        if (clockUI) clockUI.textContent = t;
    }, 1000);

    // [MAP] Inisialisasi Peta
    ...
    L.marker([<?= $latPegawai ?>, <?= $lngPegawai ?>]).addTo(map).bindPopup("Posisi Anda").openPopup();
    L.circle([<?= $latKantor ?>, <?= $lngKantor ?>], { radius: <?= $radiusMeter ?>, color: 'blue', fillOpacity: 0.1 }).addTo(map);
    ...
    // [AI] Load Model
    try {
        // 1. Ambil data wajah dari database untuk dicocokkan
        ...
        // Tombol biarkan tetap disabled (jangan aktifkan dulu)
        ...
        // 2. Event Listener saat video mulai berjalan (SCAN REAL-TIME)
        video.addEventListener('play', () => {
            ...
            async function scanWajah() {
                ...
                if (detect) {
                    ...
                    const confidenceScore = detect.detection.score; // AI Cek Wajah Utuh/Tertutup
                    ...
                    // Wajah harus utuh (>70%) DAN mirip (<0.45)
                    ...
                    if (confidenceScore > 0.70 && distance < 0.45 && faceRatio > 0.20) {
                        ...
                        // --- KONDISI 1: WAJAH COCOK (Animasi Sukses) ---
                        btn.disabled = false;
                        ...
                        // Bingkai menyala merah bahaya
                        faceCutout.style.borderColor = "#dc3545";
                        faceCutout.style.boxShadow = "0 0 0 9999px rgba(95, 58, 58, 0.75), 0 0 25px rgba(220, 53, 69, 0.6) inset";
                    }
                } else {
                    // --- KONDISI 3: TIDAK ADA WAJAH (Animasi Standar) ---
                    btn.disabled = true;
                    info.innerHTML = "<span class='spinner-border spinner-border-sm me-2'></span> Arahkan wajah ke dalam area...";
                    info.className = "alert alert-warning py-2 small mb-3";
                    
                    // Kembali ke warna standar (Biru & Putih transparan)
                ...
                setTimeout(scanWajah, 600); // 600ms = Cukup cepat tapi ringan
            }
        ...
        info.innerHTML = "Gagal memuat AI. Periksa koneksi/izin kamera.";
        info.className = "alert alert-danger py-2 small mb-3";
    }
...
btn.addEventListener('click', () => {
    ...
    info.innerHTML = "Memproses kehadiran...";
    ...
    Swal.fire({
        ...
        text: "Sedang menyimpan data presensi...", 
        icon: "success", 
